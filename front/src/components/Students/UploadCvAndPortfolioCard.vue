...
async function handleCvUpload(e) {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/student/upload-cv`, {
      method: 'POST',
      headers: {
...
async function deleteCv() {
  const res = await fetch(`${import.meta.env.VITE_API_URL}/student/delete-cv`, {
    method: 'DELETE',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('token')}`,
    },
...
async function updateLinkLinkedin() {
  const res = await fetch(`${import.meta.env.VITE_API_URL}/student/update-linkedin`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('token')}`,
...
async function updateGithub() {
  const res = await fetch(`${import.meta.env.VITE_API_URL}/student/update-github`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('token')}`,
      'Content-Type': 'application/json',
