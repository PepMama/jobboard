...
async function save() {
  ...
  if (!token) return
  const res = await fetch(`${import.meta.env.VITE_API_URL}/student/manage-experience`, {
    method: 'POST',
...
async function del(id: number) {
  ...
  if (!t) return
  const res = await fetch(`${import.meta.env.VITE_API_URL}/student/delete-experience/${id}`, {
    method: 'DELETE',
