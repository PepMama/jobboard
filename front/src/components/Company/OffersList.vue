...
async function loadOffers() {
  ...
  try {
    ...
    const response = await fetch('https://127.0.0.1:8000/company/offers', {
      headers: {
...
async function confirmDelete() {
  ...
  try {
    ...
    const response = await fetch(`https://127.0.0.1:8000/company/delete-offer/${offerToDelete.value}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
