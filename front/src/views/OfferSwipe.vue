...
export default {
  ...
  methods: {
    async fetchOffers() {
      ...
      try {
        const response = await fetch(`https://127.0.0.1:8000/student/offers?${params.toString()}`, {
    ...
    async likeOffer(offerId) {
      const token = localStorage.getItem('token')
      try {
        const response = await fetch(`https://127.0.0.1:8000/student/like-offer/${offerId}`, {
          method: 'POST',
          headers: {
